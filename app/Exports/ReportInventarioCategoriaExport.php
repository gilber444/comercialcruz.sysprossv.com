...
class ReportInventarioCategoriaExport implements FromCollection, WithHeadings, WithTitle, WithCustomStartCell, WithStyles
{
    protected $sucursal, $categoria;

    function __construct($sucursal, $categoria)
    {
    ...
    public function collection()
    {
        ...
        if($this->sucursal == 0)
        {
            if($this->categoria == 0)
            {
                ...
                ->select('p.codebar3', 'c.categoria', 'p.nombreProducto', 'm.unidad as medida', DB::raw('"Todas las sucursales" as sucursal'), DB::raw('SUM(inventarios.existencia) as existencia'))
            ...
            else
            {
                ...
                ->join('medidas as m', 'm.id', 'p.medida')
                ->select('p.codebar3', 'c.categoria', 'p.nombreProducto', 'm.unidad as medida', DB::raw('"todas las sucursales" as sucursal'), DB::raw('SUM(inventarios.existencia) as existencia'))
                ->where('p.categoria', $this->categoria)
                ->groupBy('p.codebar3', 'p.nombreProducto', 'c.categoria', 'm.unidad')
                ->get();
            }
        }
        else
        {
            if($this->categoria == 0)
            {
                ...
                ->join('categorias as c', 'c.id', 'p.categoria')
                ->join('medidas as m', 'm.id', 'p.medida')
                ->select('p.codebar3', 'c.categoria', 'p.nombreProducto', 'm.unidad as medida', 's.nombre as sucursal', 'inventarios.existencia')
    ...
    {
        return ["Codigo de Barra", "Categoria", "Descripcion", "Medida", "Sucursal", "Existencia", "Conteo"];
    }
    ...
    {
        return 'Reporte de Inventario por Categorias';
    }
}
