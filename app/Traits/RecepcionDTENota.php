...
trait RecepcionDTENota
{
    ...
    /**
     * Envía una Nota de Crédito a recepción de Hacienda.
    ...
    public function RecepcionDTENota($id)
    {
        try {
            $user = Auth::user();
            $empresa = Empresas::find($user->empresa ?? 1);
            ...
            $firma = $this->obtenerOFirmarNota($id, $dte);
            ...
            $this->guardarRespuestaExitosaNota($id, $dte, $responseData);

        } catch (ClientException $e) {
            $this->guardarRespuestaRechazadaNota($id, $e);
            return;
        } catch (RequestException $e) {
            $this->guardarErrorRecepcionNota($id, 'Error de conexión con Hacienda: ' . $e->getMessage());
            throw $e;
        } catch (\Throwable $e) {
            $this->guardarErrorRecepcionNota($id, 'Error inesperado: ' . $e->getMessage());
            throw $e;
        }
     ...
     */
    private function obtenerOFirmarNota($id, $dte): ?string
    {
    ...
    /**
     * Guarda una respuesta exitosa de Hacienda y actualiza el DTE y la nota de crédito.
     */
    private function guardarRespuestaExitosaNota($id, dte $dte, array $responseData): void
    {
        ...
        if ($nota) {
            $nota->estado = $responseData['estado'] ?? 'ERROR';
            $nota->sello = $responseData['selloRecibido'] ?? null;
            $nota->save();
        }
    }
     ...
     */
    private function guardarRespuestaRechazadaNota($id, ClientException $e): void
    {
        ...
        $nota = NotaCredito::where('codigo', $dte?->codigoGeneracion)->first();
        if ($nota) {
            $nota->estado = $errorResponseData['estado'] ?? 'RECHAZADO';
            $nota->save();
        }
    }
     ...
     */
    private function guardarErrorRecepcionNota($id, string $mensaje): void
    {
        RecepcionDte::create([
            'dte' => $id,
            'fechaRecepcion' => Carbon::now()->toDateString(),
