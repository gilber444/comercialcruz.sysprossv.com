...
class SyncLocalAVPSTablas extends Command
{
    ...
    protected function syncTable(string $table, int $limit, string $connLocal, string $connVps, string $direction, int $sucursalInventarios = 0): array
    {
        ...
        if ($wmTs) {
            ...
            $precheck = DB::connection($connLocal)->table($table)
                ...
                ->where($tsCol, '<=', $cutoff);
            if ($table === 'dtes' && Schema::connection($connLocal)->hasColumn('dtes', 'estado')) {
                $precheck->where(function ($query) {
                    $query->whereNull('estado')
                        ->orWhereRaw("UPPER(TRIM(estado)) <> 'RECHAZADO'");
                });
            }
            $hasChanges = $precheck->limit(1)->exists();
            if (!$hasChanges) {
                return [
                    'count' => 0,
                    'ins' => 0,
        ...
        foreach ($rows as $r) {
            ...
            foreach (($plan['fk'] ?? []) as $col => $refTable) {
                if (array_key_exists($col, $payload)) {
                    ...
                    // Si no es requerida y mapFk devolvió null, conserva el valor local original
                }
            }
            // 5) Upsert en VPS por sincro_id (nunca forzar PK)
            unset($payload['id'], $payload['id_vps']); // 🔒 nunca empujar PK local
            // La columna compra_id_vps solo tiene sentido en local (guarda el id VPS de la compra).
            if ($table === 'cuentas_pagars' && array_key_exists('compra_id_vps', $payload)) {
                $payload['compra_id_vps'] = null;
            }
            ...
            // 6.1) Para compras, propagar su id_vps a las cuentas por pagar locales hijas
            if ($table === 'compras') {
                DB::connection($connLocal)->table('cuentas_pagars')
                    ->where('compra', $r->id)
                    ->update(['compra_id_vps' => $remoteId]);
            }
            // 7) Avanzar WM local
            $lastTs = $r->{$tsCol};
            $lastId = $r->id;
    ...
    protected function mapFk(string $refTable, $localId, string $connLocal, string $connVps): ?int
    {
        ...
        // 3.5) ESPECIAL: inventarios — buscar por producto+sucursal en VPS
        if ($refTable === 'inventarios') {
            $invLocal = DB::connection($connLocal)->table('inventarios')
                ->where('id', $localId)
                ->first(['producto', 'sucursal']);
            if ($invLocal) {
                $invVps = DB::connection($connVps)->table('inventarios')
                    ->where('producto', $invLocal->producto)
                    ->where('sucursal', $invLocal->sucursal)
                    ->first(['id']);
                if ($invVps) {
                    // Guardar id_vps en local para futuras veces
                    DB::connection($connLocal)->table('inventarios')
                        ->where('id', $localId)->update(['id_vps' => (int)$invVps->id]);
                    return (int) $invVps->id;
                }
            }
        }
    ...
    {
        $connLocal = (string) $this->option('local');
        $row = DB::connection($connLocal)->table($this->syncStatesTable)
    ...
    protected function saveWatermark(string $direction, string $table, ?string $ts, int $id): void
    {
        ...
        $now  = now();
        $tsDb = $ts ?: null;
        $exists = DB::connection($connLocal)->table($this->syncStatesTable)
            ->where('direction', $direction)
            ->where('table', $table)->first();
        if ($exists) {
            DB::connection($connLocal)->table($this->syncStatesTable)
                ->where('direction', $direction)
                ->where('table', $table)
                ->update([
