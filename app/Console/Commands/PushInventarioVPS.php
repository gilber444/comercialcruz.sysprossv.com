    ...
    public function handle(): int
    {
        ...
        try {
            if (env('APP_MODO', 'local') === 'vps') {
                $this->error('❌ push:inventario-vps NO debe ejecutarse en VPS (APP_MODO=vps). Abortando.');
                return self::FAILURE;
            }

            $sucursalesProtegidas = array_values(array_filter(array_map('trim', explode(',', env('PROTECTED_SUCURSAL_IDS', ''))), fn($v) => $v !== ''));
            if (in_array((string) $sucursalId, $sucursalesProtegidas, true)) {
                $this->error("❌ La sucursal {$sucursalId} está protegida (PROTECTED_SUCURSAL_IDS). No se puede hacer push de su inventario. Abortando.");
                return self::FAILURE;
            }
