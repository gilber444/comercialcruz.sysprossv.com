...
class PosController extends Component
{
    ...
    public function Carrito()
    {
        ...
        ->where('esenario', 1)
        ->where('sucursal', session('sucursal'))->count();
        ...
        $ultimotiquet = Caja::where('cajero', Auth::user()->id)->where('sucursal', session('sucursal'))->where('caja', session('caja'))->where('empresa', session('empresa'))->latest()->first();

        if($ultimotiquet)
        {
            if($this->itemsQuantity == 0)
            {
                if($ultimotiquet->fecha == date('Y-m-d'))
                {
    ...
    public function updateUni($id)
    {
        ...
        $product = Productos::find($datos->producto);
        $this->productoName = $product->nombreProducto;
        $this->detallePrecios = Precios::where('producto', $product->id)->where('escala', 'No')
        ->orderBy('cantidad', 'asc')
        ->get();
    ...
    {
        ////validar ventas a 0.00
        $caja0 = Caja::select('total')->where('caja', session('caja'))
        ->where('sucursal', session('sucursal'))
        ->where('empresa', session('empresa'))->where('fecha', date('Y-m-d'))->where('total', 0.0000)->delete();

        $venta0 = Ventas::select('total')->where('caja', session('caja'))
        ->where('sucursal', session('sucursal'))
        ->where('empresa', session('empresa'))->where('fecha', date('Y-m-d'))->where('total', 0.0000)->delete();

        $ventasSinDetalle = Ventas::where('caja', session('caja'))
            ->where('sucursal', session('sucursal'))
            ->where('empresa', session('empresa'))
            ->where('fecha', date('Y-m-d'))
            ->doesntHave('RdetalleVentas')
            ->get();

        if ($ventasSinDetalle->count() > 0) {
            foreach ($ventasSinDetalle as $venta) {

                $caja = Caja::where('venta', $venta->id)->first();
                if($caja){
        ...
        try {
            ...
            if ($ingre->total <> 0) {
                ...
                if($para->dte == 'Si' && $enviar)
                {
                    ...
                    $discountFactor  = (100 - $discountPercent) / 100; // 0.8

                    $detalleDTE = resumenDte::create([
                        'dte' => $dte->id,
                        'totalNoSuj' => 0,
                        'totalExenta' => 0,
                        'totalGravada' => (($sub + $descuentos) * $discountFactor),
                        'totalIva' => ($tiva * $discountFactor),
                        'subTotalVentas' => $sub * $discountFactor,
                        'descuNoSuj' => 0,
                        'descuExenta' => 0,
                        'descuGravada' => 0,
                        'porcentajeDescuento' => 0,
                        'totalDescu' => ($descuentos * $discountFactor),
                        'tributo' => $tributo->id,
                        'codigo' => null,
                        'descripcion' => null,
                        'valor' => null,
                        'subTotal' => ($sub * $discountFactor),
                        'ivaPerci1' => ($percecion * $discountFactor),
                        'ivaRete1' => 0,
                        'reteRenta' => 0,
                        'montoTotalOperacion' => (($sub -  $percecion) * $discountFactor),
                        'totalNoGravado' => 0,
                        'totalPagar' => (($sub -  $percecion) * $discountFactor),
                        'totalLetras' => strtoupper(Convertidor::montoALetras(round((($sub - $percecion) * $discountFactor), 2))),
                        'saldoFavor' => 0,
        ...
        try {
            ...
            if ($ingre) {
                foreach ($cartItems as $item) {
                    ...
                    $des = 'Venta con Factura numero ' . $correlativo;$des = 'Venta con tikect numero ' . $correlativo. " realizado por " . Auth::user()->name;
                    $kardex = Kardex::create([
                        'producto' => $item->producto,
                        'inventario' => $in->id,
                        'descripcion' => $des,
    ...
    public function ScanCodeById($barcode)
    {
        ...
        $this->productoName = $product->nombreProducto;
        $this->detallePrecios = Precios::where('producto', $product->id)
        ->where('escala', 'No')
        ->orderBy('cantidad', 'asc')
        ->get();

         $this->detalleEscalas = Precios::where('producto', $product->id)
        ->where('escala', 'Si')
        ->orderBy('cantidad', 'asc')
        ->get();
