...
class PosController extends Component
{
    ...
    public function GenerarDTE($id)
    {
        try {
            ...
            $caja = Caja::where('venta', $dte2->venta)
                ->where('codigo', $dte2->codigoGeneracion)
                ->first();

            $ticketData = $caja ? $this->TraitTikets($caja->id) : null;

            if ($dte2->estado == 'PROCESADO') {
                $this->emit('item-addedd', 'DTE Firmado y Procesado correctamente.', $ticketData);
            } elseif ($dte2->estado == 'RECHAZADO') {
                $this->emit('item-errorr', 'DTE Rechazado por Hacienda. Revise el DTE generado para más información.', $ticketData);
            } else {
                $this->emit('item-errorr', 'El DTE no pudo ser procesado. Estado actual: ' . $dte2->estado, $ticketData);
            }
        } catch (\Throwable $e) {
