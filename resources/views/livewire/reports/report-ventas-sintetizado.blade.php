<div>
    ...
    <div class="card shadow-sm mb-4">
                ...
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div style="color:#fff;">
                        ...
                        <div style="font-size:11px; opacity:.8; margin-top:1px;">
                            Resumen de ventas agrupado por sucursal y período
                        </div>
                    </div>
                </div>
        ...
        <div class="card-body py-3 px-3">
            ...
            <div class="d-flex align-items-center gap-2 mt-3 flex-wrap">
                ...
                @if(count($data) > 0)
                    @php
                        $sucursalP   = $sucursal   ?? '0';
                        $facturadorP = $facturador ?? '0';
                        $f1 = ($reporteType == 1 && $dateFrom) ? $dateFrom : now()->format('Y-m-d');
                        $f2 = ($reporteType == 1 && $dateTo)   ? $dateTo   : now()->format('Y-m-d');
                    @endphp
                    <a href="{{ url('report/pdfVentasSintetizado/' . $sucursalP . '/' . $reporteType . '/' . $facturadorP . '/' . $f1 . '/' . $f2) }}"
                        class="btn btn-sm btn-label-danger rounded-pill px-3" target="_blank">
                        <i class="fa-solid fa-file-pdf me-1"></i> PDF
                    </a>
                    <a href="{{ url('report/excelVentasSintetizado/' . $sucursalP . '/' . $reporteType . '/' . $facturadorP . '/' . $f1 . '/' . $f2) }}"
        ...
        <div class="col-6 col-md-4">
            <div class="card shadow-sm h-100" style="border-left:4px solid #233446;">
                <div class="card-body py-3 px-3">
                    <div class="d-flex align-items-center justify-content-between">
                        <div>
                            <div class="text-muted small fw-semibold text-uppercase" style="font-size:10px; letter-spacing:.5px;">Sucursales</div>
                            <div class="fw-bold mt-1" style="font-size:18px;">{{ count($data) }}</div>
                        </div>
        ...
        <div class="col-6 col-md-4">
            <div class="card shadow-sm h-100" style="border-left:4px solid #4a90d9;">
                <div class="card-body py-3 px-3">
                    <div class="d-flex align-items-center justify-content-between">
                        <div>
                            <div class="text-muted small fw-semibold text-uppercase" style="font-size:10px; letter-spacing:.5px;">Total Transacciones</div>
                            <div class="fw-bold mt-1" style="font-size:18px; color:#4a90d9;">{{ number_format($data->sum('total_ventas'), 0) }}</div>
                        </div>
    ...
    <div class="card shadow-sm">
        <div class="card-header border-bottom py-2 px-3 d-flex align-items-center justify-content-between">
            <span class="fw-bold small"><i class="fa-solid fa-list me-2 text-primary"></i>Resumen por sucursal</span>
            ...
            <table class="table table-hover table-sm align-middle mb-0" style="font-size:.82rem;">
                <thead style="position:sticky; top:0; z-index:10;">
                    <tr style="background:#233446; color:#fff;">
                        ...
                        <th class="text-center" style="width:110px;">Cant. Ventas</th>
                        <th class="text-end pe-3" style="width:130px;">Total Ventas</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($data as $i => $r)
                        <tr>
                            ...
                            <td class="fw-semibold small">{{ $r->sucursal }}</td>
                            <td class="text-center small">{{ \Carbon\Carbon::parse($dateFrom)->format('d/m/Y') }}</td>
                            <td class="text-center small">{{ \Carbon\Carbon::parse($dateTo)->format('d/m/Y') }}</td>
                            <td class="text-center">
                                <span class="badge bg-label-primary rounded-pill">{{ $r->total_ventas }}</span>
                            </td>
                            <td class="text-end pe-3 fw-semibold">$ {{ number_format($r->total_monto, 2) }}</td>
                        </tr>
                    @empty
                        <tr>
                ...
                <tfoot>
                    <tr style="background:#233446; color:#fff; font-weight:700; font-size:.8rem;">
                        ...
                        <td class="text-end pe-3">$ {{ number_format($totalSales, 2) }}</td>
                    </tr>
                </tfoot>
                @endif
            </table>
