<div>
                ...
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div style="color:#fff;">
                        ...
                        <div style="font-size:11px; opacity:.8; margin-top:1px;">
                            Consulta y análisis de ventas por sucursal, caja y período
                        </div>
                    </div>
                </div>
        ...
        <div class="card-body py-3 px-3">
            <div class="row g-2 align-items-end">
                ...
                <div class="col-sm-6 col-md-2">
                    ...
                    <select class="form-select form-select-sm" wire:model.defer="caja">
                        <option value="0">Todas</option>
                        @foreach ($cajas as $c)
            ...
            <div class="d-flex align-items-center gap-2 mt-3 flex-wrap">
                ...
                @if(count($data) > 0)
                    ...
                    @endphp
                    <a href="{{ url('report/pdfVentas/' . $sucursalP . '/' . $cajaP . '/' . $reporteType . '/' . $facturadorP . '/' . $f1 . '/' . $f2) }}"
                        class="btn btn-sm btn-label-danger rounded-pill px-3" target="_blank">
                        <i class="fa-solid fa-file-pdf me-1"></i> PDF
                    </a>
                    <a href="{{ url('report/excelVentas/' . $sucursalP . '/' . $cajaP . '/' . $reporteType . '/' . $facturadorP . '/' . $f1 . '/' . $f2) }}"
        ...
        <div class="col-6 col-md-3">
            <div class="card shadow-sm h-100" style="border-left:4px solid #233446;">
                <div class="card-body py-3 px-3">
                    <div class="d-flex align-items-center justify-content-between">
                        <div>
                            <div class="text-muted small fw-semibold text-uppercase" style="font-size:10px; letter-spacing:.5px;">Transacciones</div>
        ...
        <div class="col-6 col-md-3">
            <div class="card shadow-sm h-100" style="border-left:4px solid #4a90d9;">
                <div class="card-body py-3 px-3">
                    <div class="d-flex align-items-center justify-content-between">
                        <div>
                            <div class="text-muted small fw-semibold text-uppercase" style="font-size:10px; letter-spacing:.5px;">Promedio por venta</div>
                            <div class="fw-bold mt-1" style="font-size:18px; color:#4a90d9;">
                                $ {{ count($data) > 0 ? number_format($totalSales / count($data), 2) : '0.00' }}
                            </div>
                        </div>
    ...
    <div class="card shadow-sm">
        <div class="card-header border-bottom py-2 px-3 d-flex align-items-center justify-content-between">
            <span class="fw-bold small"><i class="fa-solid fa-list me-2 text-primary"></i>Detalle de ventas</span>
            ...
            <table class="table table-hover table-sm align-middle mb-0" style="font-size:.82rem;">
                <thead style="position:sticky; top:0; z-index:10;">
                    <tr style="background:#233446; color:#fff;">
                        ...
                        <th class="text-center" style="width:120px;">Facturador</th>
                        <th>Cliente</th>
                ...
                <tbody>
                    @forelse ($data as $i => $r)
                        <tr>
                            ...
                            <td class="text-center">
                                <span class="text-muted small">{{ $r->codigo ?? '—' }}</span>
                            </td>
                            ...
                            <td class="text-center">
                                <span class="badge bg-label-secondary rounded-pill" style="font-size:10px;">
                                    {{ $r->facturadors }} — {{ $r->correlativo }}
                                </span>
                            </td>
                            <td class="fw-semibold small">{{ $r->cliente }}</td>
                            <td class="text-center">
                                <button class="btn btn-sm btn-label-primary rounded-pill px-2" style="font-size:11px;"
                                    wire:click.prevent="getDetails({{ $r->id_venta }})"
    ...
    @include('livewire.reports.ventas-detalle')
</div>
