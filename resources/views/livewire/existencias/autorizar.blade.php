    ...
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                @if (trim($estados) === 'Solicitado')
                    <button type="button" wire:click.prevent="Update()" class="btn btn-primary"><i
                            class='bx bx-revision'></i> Actualizar Datos {{ $estados }}</button>
                @elseif($estado === 'Autorizado')
                    <button type="button" wire:click.prevent="DespacharTodos({{ $selected_id }})"
                        class="btn btn-primary"><i class="fa-solid fa-truck"></i> Despachar Todos</button>
                @else
                    <button type="button" wire:click.prevent="Finalizar({{ $selected_id }})"
                        class="btn btn-label-success" data-bs-dismiss="modal">Cerrar {{ $estados }}</button>
                @endif
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-sm-12 col-md-6 mb-2">
                        <div class="input-group">
                            <label class="input-group-text" for="inputGroupSelect01">Solicitud No.
                                {{ $numero }}</label>
                        </div>
                    </div>
                    <div class="col-sm-12 col-md-6 mb-2">
                ...
                <div class="row">
                    <div class="col-sm-12 col-md-6 mb-2">
                        <div class="input-group">
                            <div class="input-group">
                                <label class="input-group-text" for="inputGroupSelect01">Solicitante:
                                    {{ $origenes }}</label>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-12 col-md-6 mb-2">
                        <div class="input-group">
                            <div class="input-group">
                                <label class="input-group-text" for="inputGroupSelect01">Pedido a:
                                    {{ $destinos }}</label>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-12 col-md-12 mb-2">
                        <div class="input-group">
                            <div class="input-group">
                                <label class="input-group-text" for="inputGroupSelect01">Detalle de la Solicitud:
                                    {{ $solicitud }}</label>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <table class="table table-hover table-sm">
                                        ...
                                        <tbody>
                                            @forelse ($detalle as $item)
                                                <tr>
                                                    <td class="text-center">{{ $item->Rproductos->codebar3 }}</td>
                                                    ...
                                                    <td class="text-center">
                                                        @if ($item->estado == 'Solicitado')
                                                            <input type="text" class="form-control input-mini"
                                                                wire:model.lazy="cantidad.{{ $item->id }}"
                                                                value="{{ $item->cantidad }}"
                                                    ...
                                                    <td>
                                                        @if ($item->estado == 'Solicitado')
                                                            <select
                                                                wire:model.lazy="selectedDestination.{{ $item->id }}"
                                                                class="form-control">
                                                                <option value="{{ $sid }}">
                                                                    {{ $snombre }}</option>
                                                    ...
                                                    <td class="text-center">
                                                        @if ($item->estado === 'Solicitado')
                                                            <button wire:click.prevent="Aprobar({{ $item->id }})"
                                                                type="button" class="btn btn-sm btn-label-success">
                                                                <i class="fa-solid fa-check"></i> Autorizar
                                                            </button>
                                                        @elseif($item->estado === 'Autorizado')
                                                            <button wire:click.prevent="Despachar({{ $item->id }})"
                                                                type="button" class="btn btn-sm btn-label-warning">
                                                                <i class="fa-solid fa-truck"></i> Despachar
                                                            </button>
                                                        @elseif($item->estado === 'Despachado')
                                                            <button type="button"
                                                                wire:click.prevent="Ingresar({{ $item->id }})"
                                                                class="btn btn-label-success">
                                                                <i class="fa-solid fa-upload"></i> Ingresar
                                                            </button>
                                                        @else
                                                            <!-- Estado desconocido -->
                                                        @endif
                                ...
                                <div class="alert alert-primary mb-4" role="alert">
                                    <div class="d-flex gap-3 text-center">
                                        <div class="flex-shrink-0">
                                            <span
                                                class="badge badge-center rounded-pill bg-primary border-label-primary p-5 me-2">
...
<script>
    document.addEventListener('livewire:load', function() {
        Livewire.on('refreshModal', () => {
            // Opcional: Mostrar un mensaje de actualización
