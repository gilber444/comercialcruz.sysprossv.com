...
<head>
    <meta charset="UTF-8">
    <title>Reporte de Ventas Sintetizado</title>
    <style>
...
<body>
    ...
    <div class="title">Reporte de Ventas Sintetizado</div>
    <div class="subtitle">
    ...
    <table>
        <thead>
            <tr>
                ...
                <th class="text-center" style="width:15%;">Desde</th>
                <th class="text-center" style="width:15%;">Hasta</th>
                <th class="text-center" style="width:15%;">Cant. Ventas</th>
                <th class="text-right" style="width:20%;">Total Ventas</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($data as $d)
            <tr>
                <td>{{ $d->sucursal }}</td>
                <td class="text-center">{{ \Carbon\Carbon::parse($from)->format('d/m/Y') }}</td>
                <td class="text-center">{{ \Carbon\Carbon::parse($to)->format('d/m/Y') }}</td>
                <td class="text-center">{{ $d->total_ventas }}</td>
                <td class="text-right">$ {{ number_format($d->total_monto, 2) }}</td>
            </tr>
            @endforeach
        </tbody>
        <tfoot>
